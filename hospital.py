...
if page == "📊 Overview":
    ...
    for _, row in st.session_state.patients.head().iterrows():
        st.markdown(
            f"<div style='background:#fff;padding:8px;border-radius:10px;margin-bottom:6px;'>"
            f"<b>{row['Name']}</b> ({row['Age']} yrs) - {row['Department']} "
            f"<span class='status {row['Status'].lower()}'>{row['Status']}</span></div>",
            unsafe_allow_html=True
        )
